...
function _gitmulti_list_all_branches() {
    ...
    local count=$(cd "$(dirname "${cur}")" && count_all_branches "$(basename "${cur}")")
    if [ "$count" -gt 50 ]; then
        read -p "There are more than 50 branches. Do you want to continue? (y/n) " choice
        case "$choice" in
            y|Y ) COMPREPLY=( $(cd "$(dirname "${cur}")" && list_all_branches "$(basename "${cur}")") );;
            * ) return 0;;
        esac
    else
        COMPREPLY=( $(cd "$(dirname "${cur}")" && list_all_branches "$(basename "${cur}")") )
    fi
    return 0
}
